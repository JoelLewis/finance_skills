...
## Purpose
Evaluate investment performance on a risk-adjusted basis using industry-standard ratios and capture analysis. This skill covers the Sharpe, Sortino, Information, Treynor, and Calmar ratios, the Omega ratio, upside/downside capture ratios, and M-squared (Modigliani-Modigliani). These metrics allow fair comparison across strategies with different risk profiles.
...
In practice, this is computed discretely as:

```
Omega(tau) = sum(max(R_i - tau, 0)) / sum(max(tau - R_i, 0))
```

Omega > 1 means expected gains above tau exceed expected losses below tau. Unlike the Sharpe ratio, Omega captures the full shape of the distribution (skewness, kurtosis).
...
```
M^2 = R_f + SR_p * sigma_b = R_f + ((R_p - R_f) / sigma_p) * sigma_b
```
...
| Calmar Ratio | CAGR / abs(MaxDD) | Return per unit of drawdown risk |
...
```
IR = (0.10 - 0.08) / 0.04 = 0.02 / 0.04 = 0.50
```
...
- **Survivorship bias:** When comparing managers, databases that exclude defunct funds overstate average performance metrics.
